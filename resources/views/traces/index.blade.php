...
@section('heading', $project->name . ' · Traces')

@section('content')
    <p class="mb-5 text-sm text-slate-500">
        Each trace is an <span class="text-slate-300">entry point</span> — an HTTP request, console command,
        scheduled task or queued job. Open one to see its full timeline (queries, cache, logs, mail, outgoing HTTP…).
    </p>
    ...
    <div class="overflow-hidden rounded-xl border border-ink-700 bg-ink-850">
        @if($traces->isEmpty())
            <p class="px-4 py-16 text-center text-sm text-slate-600">No traces captured yet</p>
        @else
                    ...
                    @foreach($traces as $t)
                        <tr class="border-t border-ink-700/70 first:border-0 hover:bg-ink-800">
                            ...
                            <td class="px-2 py-3 text-right">
                                @if($t['errored'])<span class="rounded bg-rose-500/10 px-1.5 py-0.5 text-[10px] font-medium text-rose-400">error</span>@endif
                            </td>
