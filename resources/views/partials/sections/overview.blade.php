...
<div class="grid gap-5 lg:grid-cols-3">
    ...
    <div class="space-y-5 lg:col-span-2">
        <div class="grid gap-4 sm:grid-cols-2">
            <div class="rounded-xl border border-ink-700 bg-ink-850 p-4">
                <div class="mb-3 flex items-center justify-between">
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-500">Throughput</span>
            ...
            <div class="rounded-xl border border-ink-700 bg-ink-850 p-4">
                <div class="mb-3 flex items-center justify-between">
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-500">p95 latency</span>
        ...
        {{-- Top routes --}}
        @include('warden::partials.card-open', ['title' => 'Top routes', 'action' => ['Requests', route('warden.project.section', ['project' => $project->slug, 'section' => 'requests', 'range' => $range])]])
            @include('warden::partials.route-table', ['routes' => $routes])
        @include('warden::partials.card-close')

        {{-- Slow queries --}}
        @include('warden::partials.card-open', ['title' => 'Slowest queries', 'action' => ['Queries', route('warden.project.section', ['project' => $project->slug, 'section' => 'queries', 'range' => $range])]])
            @include('warden::partials.query-table', ['queries' => $slow])
        @include('warden::partials.card-close')

        {{-- Queues --}}
        @include('warden::partials.card-open', ['title' => 'Queues', 'action' => null])
    ...
    <div class="space-y-5">
        @if($incidents->where('status', 'open')->count())
            <div class="rounded-xl border border-rose-500/30 bg-rose-500/5 p-4">
                <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-rose-400">Active incidents</p>
                <div class="space-y-2">
        ...
        @include('warden::partials.card-open', ['title' => 'Recent issues', 'action' => ['All', route('warden.issues', $project->slug)]])
            @forelse($recent_issues as $issue)
                <a href="{{ route('warden.issue', [$project->slug, $issue->id]) }}" class="flex items-start gap-3 border-t border-ink-700 px-4 py-3 first:border-0 hover:bg-ink-800">
            ...
            @empty
                <p class="px-4 py-6 text-center text-sm text-slate-600">No open issues 🎉</p>
            @endforelse
        @include('warden::partials.card-close')

        @include('warden::partials.card-open', ['title' => 'Heartbeats', 'action' => null])
            @forelse($heartbeats as $hb)
                <div class="flex items-center gap-2.5 border-t border-ink-700 px-4 py-2.5 text-sm first:border-0">
            ...
            @empty
                <p class="px-4 py-6 text-center text-sm text-slate-600">No heartbeats tracked</p>
            @endforelse
        @include('warden::partials.card-close')

        @include('warden::partials.card-open', ['title' => 'Recent traces', 'action' => ['All', route('warden.traces', $project->slug)]])
