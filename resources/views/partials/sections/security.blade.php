...
@if(session('warden_status'))
    <div class="mb-4 rounded-lg border border-emerald-700/50 bg-emerald-900/20 px-4 py-2.5 text-sm text-emerald-300">{{ session('warden_status') }}</div>
@endif
...
@can('manageWarden')
    <div class="mb-4 flex justify-end">
        <form method="POST" action="{{ route('warden.admin.projects.audit-now', $project->id) }}">
            @csrf
            <button class="rounded-lg border border-ink-600 px-3 py-1.5 text-sm text-slate-300 transition hover:border-brand-500 hover:text-white">Run audit now</button>
        </form>
    </div>
@endcan
...
@if($audit === null)
    <div class="rounded-xl border border-dashed border-ink-700 bg-ink-900 p-12 text-center">
        <p class="text-slate-400">No security audit shipped yet.</p>
        <p class="mt-1 text-sm text-slate-600">Run <code class="text-brand-400">php artisan warden:audit</code> on the child, or set an audit frequency under <span class="text-slate-400">Manage projects → Audit</span> to have the child run it automatically.</p>
    </div>
@else
    <div class="mb-5 flex flex-wrap items-center gap-3 text-sm">
        <span class="text-slate-400">Last audit {{ Format::ago($audit->occurred_at) }}</span>
        @foreach($tools as $tool => $ran)
            <span class="rounded px-1.5 py-0.5 text-[11px] {{ $ran ? 'bg-ink-700 text-slate-300' : 'bg-amber-500/10 text-amber-400' }}">{{ $tool }}: {{ $ran ? 'ran' : 'skipped' }}</span>
        @endforeach
    </div>
    ...
    <div class="mb-5 grid grid-cols-3 gap-3 sm:grid-cols-6">
        @foreach($order as $sev)
            ...
            <div class="rounded-xl border border-ink-700 bg-ink-850 p-3 text-center">
    ...
    @include('warden::partials.card-open', ['title' => 'Vulnerabilities (' . ($payload['total'] ?? 0) . ')', 'action' => null])
        @if(empty($advisories))
            <p class="px-4 py-10 text-center text-sm text-emerald-400">No known vulnerabilities 🎉</p>
        @else
            <table class="w-full text-sm">
                <thead><tr class="border-b border-ink-700 text-[11px] uppercase tracking-wider text-slate-500">
                    ...
                    <th class="px-4 py-2 text-left font-medium">Advisory</th>
                    <th class="px-4 py-2 text-left font-medium">Affected</th>
                </tr></thead>
                <tbody>
                    @foreach($advisories as $a)
                        ...
                        <tr class="border-t border-ink-700/70 hover:bg-ink-800">
                            ...
                            <td class="px-4 py-2.5 font-mono text-[11px] text-slate-400">{{ $a['affected'] ?? '' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
